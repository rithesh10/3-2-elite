...
class p1{
     static int count=0;
    public static void fun(int[] small,int[] large,int Lidx,int Sidx,int sum)
    {
        if(Sidx==small.length)
        {
            count++;
            return ;
        }
        if(Lidx>=large.length)
        {
            return ;
        }
        // int count=0;
        for(int i=Lidx;i<large.length;i++)
        {
           
            if(large[i]+small[Sidx]>=sum)
            {
                fun(small,large,i+1,Sidx+1,large[i]+small[Sidx]);
            }
